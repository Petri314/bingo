...
setIsAdmin(true);
  ...
  <style>
    *{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;}
    ...
    .grid-container{display:grid;grid-template-columns:1fr 1fr;gap:2mm;width:fit-content;margin:0 auto;padding-top:15mm;}
  ...
  cardsToPrint.forEach((c, idx) => {
    if (idx % 4 === 0) {
      if (idx > 0) html += `</div>`;
      html += `<div class="grid-container" style="page-break-before:${idx === 0 ? 'auto' : 'always'};">`;
    }
  ...
  html += `</div><div style="height:1px;"></div></body></html>`;
    ...
    <div key={letter} className="bingo-row" style={{ display:"flex", gap:3, marginBottom:rowIdx < 4 ? 3 : 0, alignItems:"center" }}>
      <div className="bingo-letter" style={{ width:34, height:34, borderRadius:8, flexShrink:0, background:BINGO_LETTER_COLORS[rowIdx], display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Poller One',cursive", fontWeight:900, fontSize:17, color:"#fff", boxShadow:`0 0 10px ${BINGO_LETTER_COLORS[rowIdx]}99` }}>{letter}</div>
          ...
          <div className="panel-inferior-pc">
            ...
            <div className="zona-patron" style={{ background: alreadyBinguito ? "#334155" : "rgba(255,255,255,0.07)", borderRadius:14, padding:"12px 10px", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:8, border: alreadyBinguito ? "1px solid #475569" : `1px solid ${binguitoColor}44`, flex:1, minHeight:0, opacity: alreadyBinguito ? 0.5 : 1 }}>
...
{winners.length === 0
  ...
  <div style={{ position:"fixed", inset:0, zIndex:400, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(0,0,0,0.6)", backdropFilter:"blur(4px)" }}>
    <div style={{ background:"#1a1d2b", borderRadius:24, padding:"32px 40px", textAlign:"center", border:`3px solid ${gc}`, boxShadow:`0 0 60px ${gc}88`, animation:"salePopIn 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards" }}>
      ...
      {toast && <div style={{ position:"fixed", top:16, left:"50%", transform:"translateX(-50%)", background:toast.type === "err" ? "#ef4444" : "#10b981", color:"#fff", padding:"12px 24px", borderRadius:12, fontWeight:700, fontSize:14, zIndex:999, boxShadow:"0 10px 15px -3px rgba(0,0,0,0.1)", whiteSpace:"nowrap", fontFamily:"sans-serif" }}>{toast.msg}</div>}
